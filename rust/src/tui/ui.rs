...
fn render_header(f: &mut Frame, app: &App, area: Rect) {
    ...
    let search_content = if app.search_active {
        ...
        Line::from(vec![
            Span::styled(" Filter: ", Style::default().fg(app.theme.secondary)),
            Span::styled(&app.search_query, Style::default().fg(app.theme.text)),
...
fn render_snippet_list(f: &mut Frame, app: &App, area: Rect) {
    ...
    let items: Vec<ListItem> = app
        ...
        .map(|(disp_idx, &actual_idx)| {
            ...
            let line1 = Line::from(vec![
                ...
                Span::styled(format!("{:<11}", snip.id), id_style),
...
fn render_footer(f: &mut Frame, app: &App, area: Rect) {
    if let Some(status) = app.active_status() {
        let status_p = Paragraph::new(Line::from(vec![
            ...
            Span::styled(status, Style::default().fg(app.theme.text)),
        ]));
        f.render_widget(status_p, area);
    } else {
        let keys = Line::from(vec![
            Span::styled(" j/k ", Style::default().bg(app.theme.surface).fg(app.theme.primary).add_modifier(Modifier::BOLD)),
            Span::raw(" Nav  "),
            ...
            Span::raw(" Delete  "),
            Span::styled(" ? ", Style::default().bg(app.theme.surface).fg(app.theme.accent).add_modifier(Modifier::BOLD)),
            Span::raw(" Help  "),
...
fn render_help_modal(f: &mut Frame, app: &App, area: Rect) {
    ...
    let help_text = vec![
        Line::from(vec![Span::styled("CPKB Keybindings", Style::default().fg(app.theme.primary).add_modifier(Modifier::BOLD))]),
        Line::from(""),
        ...
        Line::from(vec![Span::styled("  g / G         ", Style::default().fg(app.theme.secondary)), Span::raw("Jump to top / bottom")]),
        Line::from(vec![Span::styled("  /             ", Style::default().fg(app.theme.secondary)), Span::raw("Search and fuzzy filter snippets")]),
        Line::from(vec![Span::styled("  c             ", Style::default().fg(app.theme.secondary)), Span::raw("Copy selected snippet code to clipboard")]),
        Line::from(vec![Span::styled("  [ / ]         ", Style::default().fg(app.theme.secondary)), Span::raw("Decrease / increase left panel width")]),
        Line::from(vec![Span::styled("  d             ", Style::default().fg(app.theme.error)), Span::raw("Delete selected snippet")]),
        Line::from(vec![Span::styled("  Esc           ", Style::default().fg(app.theme.text_dim)), Span::raw("Clear search / close modal")]),
        Line::from(vec![Span::styled("  q             ", Style::default().fg(app.theme.text_dim)), Span::raw("Quit CPKB")]),
        Line::from(""),
        Line::from(vec![Span::styled("Press Esc or ? to close this help window.", Style::default().fg(app.theme.text_dim))]),
    ];
    ...
    let help_p = Paragraph::new(help_text)
        .block(
            Block::default()
                .borders(Borders::ALL)
                .border_type(BorderType::Rounded)
                .title(Span::styled(" Help ", Style::default().fg(app.theme.primary).add_modifier(Modifier::BOLD)))
