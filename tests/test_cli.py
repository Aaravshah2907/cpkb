...
def test_cmd_setup_yes_creates_config_and_directories(temp_db, capsys):
    ...
    raw_saved = json.loads((db.APP_DIR / "config.json").read_text(encoding="utf-8"))
    assert raw_saved["editor"]["command"] == ""
    assert raw_saved["display"]["left_pane_width"] == 35
    assert raw_saved["display"]["custom_theme"]["primary"] == "#00ffff"
...
def test_cmd_setup_reset_config_uses_factory_defaults(temp_db):
    ...
    raw_saved = json.loads((db.APP_DIR / "config.json").read_text(encoding="utf-8"))
    assert raw_saved["editor"]["command"] == ""
    assert raw_saved["display"]["layout"] == "horizontal"
...
def test_cmd_import_markdown_accepts_language_fence(temp_db):
    """Test markdown import accepts files generated by language-tagged exports."""
    md = b"""## CPP Title (CPP001)
**Description:** Desc
**Use case:** Use
**Tags:** tag
...
```
"""
    md_path = db.APP_DIR / "imports" / "snippets_cpp.md"
    md_path.write_bytes(md)

    args = MagicMock()
    args.list_defaults = False
    args.defaults = False
    args.source = str(md_path)
    args.format = None
    args.encrypted = False
    args.regenerate_ids = False
    cli.cmd_import(args)
    ...
    cursor = temp_db.cursor()
    cursor.execute("SELECT title, code FROM snippets WHERE id = ?", ("CPP001",))
    assert cursor.fetchone() == ("CPP Title", "cout << 3;")


def test_cmd_export_markdown_round_trips_through_import(temp_db):
    """Test markdown export output can be imported back into an empty database."""
    cpkb_config.save_config(
        db.APP_DIR,
        {
            "snippets": {
    ...
    cursor = temp_db.cursor()
    db.add_snippet(cursor, temp_db, "Round Trip", "desc", "use", "tag", "cout << 4;")

    cli.cmd_export(MagicMock())
    export_path = next((db.APP_DIR / "exports").glob("snippets_*.md"))
    ...
    args.list_defaults = False
    args.defaults = False
    args.source = str(export_path)
    args.format = None
    args.encrypted = False
    args.regenerate_ids = False
    cli.cmd_import(args)

    cursor.execute("SELECT title, code FROM snippets")
    assert cursor.fetchone() == ("Round Trip", "cout << 4;")
...
def test_get_snippet_fields_rejects_unknown_columns(temp_db):
    """Test dynamic snippet field selection is limited to known columns."""
    cursor = temp_db.cursor()
    snippet_id = db.add_snippet(cursor, temp_db, "Safe Fields", "desc", "use", "tag", "code")

    assert db.get_snippet_fields(cursor, snippet_id, ["id", "title"]) == (snippet_id, "Safe Fields")
    with pytest.raises(ValueError, match="Invalid snippet field"):
        db.get_snippet_fields(cursor, snippet_id, "id, sqlite_version()")
